...
async function sendEmail(to, subject, html, attachments = []) {
  ...
  const mailOptions = {
    from: '"Richards & Law" <MS_44186f2@test-y7zpl98dwmr45vx6.mlsender.net>',
    to: to,
    subject: subject,
    html: html,
  };
  ...
  try {
    ...
    console.error('❌ MailerSend SMTP Error:', error.message);
...
async function sendVerificationEmail(extractedData, verificationToken, matterId) {
  ...
  console.log('✅ Verification email sent via MailerSend SMTP');
}
...
async function sendClientEmail(clientEmail, clientName, extractedData, solDate, bookingLink, matterId, retainerAttachment) {
  ...
  console.log('✅ Client email sent via MailerSend SMTP');
}
