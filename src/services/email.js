...
async function sendEmail(to, subject, html, attachments = []) {
  const emailParams = {
    from: 'Richards & Law <onboarding@trial-xxxxx.mailersend.net>', // Replace with your verified sender
    to: [to],
    subject: subject,
    html: html,
  };
  ...
  if (attachments.length > 0) {
    emailParams.attachments = attachments.map(a => ({
      filename: a.filename,
      content: Buffer.isBuffer(a.content) ? a.content.toString('base64') : a.content.toString('base64'),
    }));
  }
...
async function sendClientEmail(clientEmail, clientName, extractedData, solDate, bookingLink, matterId, retainerAttachment) {
  ...
  console.log('✅ Client email sent via MailerSend');
}
